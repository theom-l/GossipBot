...
class PullAudioOutputStream(AudioOutputStream):
    ...
    def __init__(self, stream_impl = None):
        if stream_impl is None:
            self._impl = impl.PullAudioOutputStream.create()
        elif isinstance(stream_impl, impl.PullAudioOutputStream):
            self._impl = stream_impl
        else:
            raise ValueError('cannot construct PullAudioOutputStream with the given stream_impl parameter')
