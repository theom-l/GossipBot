...
class SpeechConfig():
    ...
    def _get_impl(config_type, subscription, region, endpoint, host, auth_token,
        ...
        if _impl is not None:
            _impl.set_property("AZAC-SDK-PROGRAMMING-LANGUAGE", "Python")
...
class Connection():
    ...
    @classmethod
    def from_dialog_service_connector(cls, dialog_service_connector:DialogServiceConnector):
        """
        Gets the :class:`.Connection` instance from the specified dialog service connector.

        """
        return cls(impl.Connection.from_dialog_service_connector(dialog_service_connector._impl))
...
class PronunciationAssessmentConfig:
    ...
    def phoneme_alphabet(self, alphabet: str):
        """
        The phoneme alphabet.
        """
        self._impl.set_phoneme_alphabet(alphabet)
    ...
    def nbest_phoneme_count(self, count: int):
        ...
        """
        self._impl.set_nbest_phoneme_count(count)
...
class PronunciationAssessmentPhonemeResult:
    ...
    def __init__(self, _json):
        ...
        if 'NBestPhonemes' in _json['PronunciationAssessment']:
            self._nbest_phonemes = [PronunciationAssessmentNBestPhoneme(x) for x in _json['PronunciationAssessment']['NBestPhonemes']]
        else:
